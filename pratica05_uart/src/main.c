...
unsigned int string_para_int(char *str) {
...
void uart_imprime_numero(unsigned int num) 
{
    if (num == 0) 
    {
        uart_putc('0');
...
int _main(void)
{
    ...
    char buffer[10];           
    int indice_buffer = 0;     
    unsigned int frequencia_hz = 1;
    
    // Calcula o período 
    unsigned int periodo_atual = 500000 / frequencia_hz; 
    ...
    uart_puts("\r\nSistema iniciado\r\n");
    uart_puts("Digite a frequencia desejada em Hz e pressione Enter:\r\n");
    ...
    while(1)
    {
        ...
        // 2. Lógica Não-Bloqueante da UART
   if(uart_rx_ready())
        {
            char c = uart_getc();
...
if(c == '\r' || c == '\n') 
{
    ...
    if(indice_buffer > 0) 
    {
        // valor digitado como MILISSEGUNDOS
        unsigned int tempo_ms = string_para_int(buffer); 

        if(tempo_ms > 0) 
        {
            periodo_atual = tempo_ms * 500;

            unsigned int freq_vezes_100 = 100000 / tempo_ms;
            unsigned int freq_inteira = freq_vezes_100 / 100;
            unsigned int freq_decimal = freq_vezes_100 % 100;

            uart_puts("\r\nNovo tempo aplicado\r\n");
            
            uart_puts("Frequencia equivalente: ");
            uart_imprime_numero(freq_inteira);
            uart_putc('.');
            if (freq_decimal < 10) 
            {
                uart_putc('0'); 
            }
            uart_imprime_numero(freq_decimal);
            uart_puts(" Hz\r\n");
            

            uart_puts("\r\nDigite o proximo tempo em ms: ");
            
            ultimo_toggle = agora; 
        }
        else 
        {
            uart_puts("\r\nErro: O tempo deve ser maior que 0 ms.\r\n");
            uart_puts("Digite o proximo tempo em ms: ");
        }
