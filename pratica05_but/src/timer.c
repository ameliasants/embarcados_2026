...
/* A função de Atraso que respeita o relógio contínuo */
void uDelay (unsigned int us){
    /* Marca o momento em que o delay começou */
    unsigned int inicio = DMTimerCounterGet(SOC_DMTIMER_7_REGS);
    unsigned int ticks_necessarios = us * TIMER_1US_COUNT;
    
    /* Fica em loop APENAS olhando a diferença, sem desligar o hardware! */
    while((DMTimerCounterGet(SOC_DMTIMER_7_REGS) - inicio) < ticks_necessarios)
    {
        /* Espera o tempo passar sem quebrar o relógio */
    }
}