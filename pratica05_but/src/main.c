...
void iniciar()
{
    ...
    /* 1. HABILITA CLOCK DO GPIO1 E TIMER7 */
    ...
    HWREG(SOC_GPIO_1_REGS + GPIO_CLEARDATAOUT) = TODOS_LEDS | LED_EXTERNO;


    /* ======================================================== */
    /* 6. CONFIGURAÇÃO DAS INTERRUPÇÕES DE HARDWARE             */
    /* ======================================================== */

    /* O cálculo 98 >> 5 resulta em 3 (MIR_CLEAR3), e 98 & 0x1F resulta no bit 2 */
    HWREG(SOC_AINTC_REGS + INTC_MIR_CLEAR3) = (1 << 2); 

    /* BOTAO 1 (Pull-up): */
    HWREG(SOC_GPIO_1_REGS + GPIO_FALLINGDETECT) |= BOTAO2; 

    /* BOTAO 2 (Pull-down) */
    HWREG(SOC_GPIO_1_REGS + GPIO_RISINGDETECT) |= BOTAO1;  

    /* Habilita a geração da interrupção nos pinos */
    HWREG(SOC_GPIO_1_REGS + GPIO_IRQSTATUS_SET_0) |= (BOTAO1 | BOTAO2);
}
...
void IntAINTCInit(void) {
    /* Reset do controlador de interrupção ARM */
    HWREG(SOC_AINTC_REGS + INTC_SYSCONFIG) = 0x2; // SOFTRESET
    
    /* Espera o reset completar */
    while((HWREG(SOC_AINTC_REGS + INTC_SYSSTATUS) & 0x1) != 0x1);
    
    /* Habilita geração de interrupção configurando o threshold */
    HWREG(SOC_AINTC_REGS + INTC_THRESHOLD) = 0xFF; 
}


/* ---------------- TRATAMENTO DE INTERRUPÇÃO ---------------- */
volatile unsigned int periodo_atual = TEMPO_PISCA_3S;
...
void ISR_Handler(void) {
    ...
    /* Reconhece a IRQ no controlador */
    HWREG(SOC_AINTC_REGS + INTC_CONTROL) = 0x1;
}

/* ---------------- TRATAMENTO DE INTERRUPÇÃO ---------------- */
void gpioIsr(void) {
    /* Lê quais pinos do GPIO1 geraram a interrupção */
    unsigned int status = HWREG(SOC_GPIO_1_REGS + GPIO_IRQSTATUS_0);


    if(status & BOTAO1) {
        periodo_atual = TEMPO_PISCA_3S;
        uart_puts("Interrupcao: BOTAO 1 pressionado - Frequencia 3s\r\n");
    }


    if(status & BOTAO2) {
        periodo_atual = TEMPO_PISCA_5S;
        uart_puts("Interrupcao: BOTAO 2 pressionado - Frequencia 5s\r\n");
    }

    /* Limpa a flag de interrupção reescrevendo o status lido */
...
int _main(void) {
    ...
    uart_puts("Sistema iniciado com Interrupcoes - Pisca LED\r\n");
